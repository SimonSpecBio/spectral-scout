...
export default async function NewEventPage({
  ...
  if (!facilityId) {
    return (
      <div className="mx-auto flex max-w-md flex-col gap-4">
        ...
        {orgFacilities.map((f) => (
          <Link key={f.id} href={`/app/new-event?facility=${f.id}`} className="card card-interactive p-4">
            {f.name}
          </Link>
        ))}
      </div>
  ...
  if (!areaId) {
    ...
    return (
      <div className="mx-auto flex max-w-md flex-col gap-4">
        ...
        {areas.map((a) => (
          <Link key={a.id} href={`/app/new-event?facility=${facilityId}&area=${a.id}`} className="card card-interactive p-4">
            {a.name}
          </Link>
        ))}
      </div>
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col gap-6">
      <h1 className="text-2xl font-semibold">New pest event</h1>
      <NewEventForm facilityId={facilityId} areaId={areaId} />
    </div>
  );
}
